...
//! ```
//!
//! A `:sni:` section opens a scope and may name several hostnames, which then share
//! the `:allow:` that follows -- the shape Envoy's ServerNameMatcher uses, where one
//! `domains` list maps to one action. An `:allow:` before any `:sni:` is the flat
//! list, which is what a listener without SNI uses.
//!
...
pub struct Config {
    ...
    pub allow: cidr::Set,
    /// Hostname scopes in config order.
...
/// A `:sni:` entry, stored ASCII-lowercased.
///
...
enum Section {
    /// Before any section: `key value` scalars.
    Head,
    Sni,
    Allow,
}
...
pub fn parse(text: &str) -> Result<Config, &'static str> {
    ...
    // (names, cidrs) per scope; the open one is the last entry.
    ...
    for raw in text.lines() {
        ...
        // `:name:` on its own line opens a section.
        if let Some(name) = line.strip_prefix(':').and_then(|r| r.strip_suffix(':')) {
            section = match name {
                ...
                "allow" => Section::Allow,
                _ => return Err("unknown section; expected :sni: or :allow:"),
            };
            continue;
        }

        match section {
            Section::Sni => match groups.last_mut() {
                Some((names, _)) => names.push(line),
                None => return Err("`:sni:` item outside a scope"),
            },
            // An `:allow:` before any `:sni:` is the flat list; after one it fills
            // the scope that `:sni:` opened.
            Section::Allow => match groups.last_mut() {
                Some((_, cidrs)) => cidrs.push(line),
                None => flat.push(line),
            },
            Section::Head => {
    ...
    let scopes = groups
        ...
        .map(|(names, cidrs)| {
            Ok(Scope {
                names: names.into_iter().map(Pattern::parse).collect(),
                allow: cidr::build_from(cidrs.into_iter())?,
            })
        })
