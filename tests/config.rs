...
fn require_ppv2_false_cannot_be_combined_with_an_allowlist() {
    ...
    assert!(config::parse(
        "ula fd2a:5c1b:7e90::/48\n\
         require_ppv2 false\n\
         allow fd2a:5c1b:7e90:1::/64\n"
    )
    .is_err());

    // Either alone is still fine.
    assert!(config::parse("ula fd2a:5c1b:7e90::/48\nrequire_ppv2 false\n").is_ok());
    assert!(config::parse("ula fd2a:5c1b:7e90::/48\nallow fd2a:5c1b:7e90:1::/64\n").is_ok());
}
...
fn a_ula_with_bits_below_slash_48_is_rejected() {
    ...
    assert!(config::parse("ula fd2a:5c1b:7e90:1234::/48\n").is_err());
    assert!(config::parse("ula fd2a:5c1b:7e90::1/48\n").is_err());
}
